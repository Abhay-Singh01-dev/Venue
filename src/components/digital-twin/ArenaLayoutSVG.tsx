...
export function ArenaLayoutSVG({
  ...
  return (
    ...
    >
      ...
      {zones.filter((z) => z.type === "concourse").map((z) => renderSection(z))}

      {/* ── Center Stage ────────────────────────────────────────── */}
      {zones.filter((z) => z.type === "field").map((z) => renderCenterStage(z))}

      {/* ── Corner Tunnel Gates ──────────────────────────────────── */}
      {zones.filter((z) => z.type === "gate").map((z) => renderTunnel(z))}
          ...
          if (zone.type === "gate") {
            return (
              <g key={`pred-${pred.zoneId}`} className="pointer-events-none">
                ...
                <text
                  x={x}
                  y={y < 275 ? y - TUNNEL_R - 16 : y + TUNNEL_R + 26}
                  textAnchor="middle"
                  fontSize={8}
                  fontWeight="bold"
                  fill={lv}
