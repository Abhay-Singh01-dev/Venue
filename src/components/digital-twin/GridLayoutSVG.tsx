...
export function GridLayoutSVG({
  ...
  return (
    <svg viewBox="0 0 900 550" className="w-full h-full">
      <defs>
        <pattern id="grid-bg-g" width="32" height="32" patternUnits="userSpaceOnUse">
      ...
      {zones.map(zone => {
        ...
        return (
          <g key={zone.id}>
            {/* Selection halo */}
            {isSelected && (
              <rect x={rx - 3} y={ry - 3} width={w + 6} height={h + 6} rx={11}
            ...
            />
            {/* Labels */}
            <text x={zone.position.x} y={zone.position.y - 5}
              textAnchor="middle" fontSize={11} fontWeight="700"
              fill={outColor} className="pointer-events-none select-none">
              {zone.shortName}
            </text>
            <text x={zone.position.x} y={zone.position.y + 10}
      ...
      {predictionMode === 'predicted' && predictions.map(pred => {
        ...
        return (
          <g key={`pred-${pred.zoneId}`} className="pointer-events-none">
            ...
            <text x={zone.position.x} y={ry + h + 14}
